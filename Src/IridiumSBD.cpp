...
// This should only be called occasionally as it will force an
// I2C transaction each time if we are using I2C
bool IridiumSBD::hasRingAsserted()
{
    ...
    {
        // If we are using serial then let's check the ringPin manually
        // instead of assuming cancelled() will be able to do it
...
{
    // Do nothing if we are using serial (the user will have to enable the charger manually)
    if (useSerial)
    {
...
{
    // Do nothing if we are using serial (the user will have to check PGOOD manually)
    if (useSerial)
    {
...
{
    // Do nothing if we are using serial (the user will have to enable the 9603N power manually)
    if (useSerial)
    {
...
{
    // Do nothing if we are using serial (the user will have to enable the 9603N manually)
    if (useSerial)
    {
...
{
    // Do nothing if we are using serial (the user will have to use hasRingAsserted instead)
    if (useSerial)
    {
...
int IridiumSBD::internalBegin()
{
    ...
    if (this->useSerial) // If we are using Serial
        // Apollo3 v2.1 Serial fix - begin the Serial port 500ms after power(true)
        beginSerialPort();

    // Turn on modem and wait for a response from "AT" command to begin
    for (unsigned long start = millis();
         !modemAlive && millis() - start < 1000UL * this->startupTimeout;)
    {
        send(F("AT\r"));
    ...
    diagprint(F("Ring alerts are"));
    diagprint(ringAlertsEnabled ? F("") : F(" NOT"));
    diagprint(F(" enabled.\r\n"));

    // This will clear ringAsserted and the Ring Indicator flag
    if (ringAlertsEnabled)
    {
        enableRingAlerts(true);
    }
    else
    {
        // If ring alerts are not enabled and using I2C,
        // make sure the Ring Indicator flag is clear
        if (!this->useSerial) clearRingIndicator();
    }
    ...
    {
        diagprint(F("Firmware version is "));
        diagprint(version);
        diagprint(F("\r\n"));
    ...
    diagprint(F("MSSTM workaround is"));
    diagprint(msstmWorkaroundRequested ? F("") : F(" NOT"));
    diagprint(F(" enforced.\r\n"));
...
int IridiumSBD::internalSendReceiveSBD(const char *txTxtMessage, const uint8_t *txData,
                                       size_t txDataSize, uint8_t *rxBuffer, size_t *prxBufferSize)
{
    ...
    if (txData && txDataSize)
    {
        ...
        else
        {
            ...
            // We need to make sure we don't send too much I2C data in one
            // go (otherwise we will overflow the ATtiny841's I2C buffer)
            size_t bytes_to_send = txDataSize; // Send this many bytes in total
            size_t i=0;
            size_t nexti;

            // If there are too many bytes to send all in one go
            while (bytes_to_send > (TINY_I2C_BUFFER_LENGTH - 3))
            {
                ...
                // Decrease the number of bytes still to send
                bytes_to_send = bytes_to_send - (TINY_I2C_BUFFER_LENGTH - 3);
                // Send data and release the bus (the 841 (WireS) doesn't
                // like it if the Master holds the bus!)
                wireport->endTransmission();
            }
            // There are now <= (TINY_I2C_BUFFER_LENGTH - 3) bytes left to send,
            // so send them and then release the bus
            wireport->beginTransmission((uint8_t)deviceaddress);
...
int IridiumSBD::internalMSSTMWorkaround(bool &okToProceed)
{
    ...
    "Before attempting any of the following commands: +SBDDET, +SBDREG, +SBDI,
     +SBDIX, +SBDIXA the field application should issue the AT command AT-MSSTM to
     the transceiver and evaluate the response to determine if it is valid or not:
    ...
    If the response is invalid, the field application should wait and recheck system
    time until a valid response is obtained before proceeding.

    This will ensure that the Iridium SBD transceiver has received a valid system time
    before attempting SBD communication.
    The Iridium SBD transceiver will receive the valid system time from the Iridium
    network when it has a good link to the satellite. Ensuring that the received signal
    strength reported in response to AT command +CSQ and +CIER is above 2-3 bars
...
// Wait for response from previous AT command.  This process terminates when "terminator"
// string is seen or upon timeout.
// If "prompt" string is provided (example "+CSQ:"), then all characters following prompt
// up to the next CRLF are stored in response buffer for later parsing by caller.
bool IridiumSBD::waitForATResponse(char *response, int responseSize, const char *prompt,
...
int IridiumSBD::doSBDRB(uint8_t *rxBuffer, size_t *prxBufferSize)
{
    ...
    if (!this->useSerial)
        check9603data(); // Check for any 9603 serial data
    ...
    {
        if(!this->useSerial)
            check9603data(); // Keep checking for new 9603 serial data
        if (cancelled())
            return ISBD_CANCELLED;
    ...
    for (uint16_t bytesRead = 0; bytesRead < size;)
    {
        ...
        if ((this->useSerial && (stream->available())) ||
            ((!this->useSerial) && i2cSerAvailable()))
        {
            uint8_t c;
            if (this->useSerial)
            {
...
void IridiumSBD::sendlong(const char *str)
{
    ...
    {
        //lastCheck = millis(); // Update lastCheck so we enforce a full I2C_POLLING_WAIT
        // We need to make sure we don't send too much I2C data in one go
        // (otherwise we will overflow the buffer)
        ...
        // If there are too many bytes to send all in one go
        while (bytes_to_send > (TINY_I2C_BUFFER_LENGTH - 1))
        {
            ...
            // Decrease the number of bytes still to send
            bytes_to_send = bytes_to_send - (TINY_I2C_BUFFER_LENGTH - 1);
            // Send data and release the bus (the 841 (WireS) doesn't like
            // it if the Master holds the bus!)
            wireport->endTransmission();
        }

        // There are now <= (TINY_I2C_BUFFER_LENGTH - 1) bytes left to send,
        // so send them and then release the bus
        wireport->beginTransmission((uint8_t)deviceaddress);
        wireport->write(DATA_REG); // Point to the 'serial register'
        for (; i<txDataSize; ++i)
           ...
           ((!this->useSerial) && (i2cSerAvailable() > 0))) && nextChar == -1)
    {
        ...
        if (*head != 0 && c == *head)
        {
            ...
            else
            {
                ...
                for (unsigned long start = millis();
                     ((this->useSerial && (stream->available() == 0)) ||
                     ((!this->useSerial) && (i2cSerAvailable() == 0))) &&
                     millis() - start < FILTERTIMEOUT; );

                if (!this->useSerial) check9603data(); // Check for new 9603 serial data

                // If there isn't one, assume this ISN'T an unsolicited SBDRING
                if ((this->useSerial && (stream->available() == 0)) ||
                    // Pop the character back into nextChar
                    ((!this->useSerial) && (i2cSerAvailable() == 0)))
                {
                    --head;
                    nextChar = c;
...
void IridiumSBD::check9603data()
{
    if (millis() - lastCheck >= I2C_POLLING_WAIT_MS)
    {
        ...
        // Send data and release the bus (the 841 (WireS) doesn't like it if
        // the Master holds the bus!)
        wireport->endTransmission();
        ...
        if (bytesAvailable > 0)
        {
            ...
            // If there are _more_ than SER_PACKET_SIZE bytes to be read
            while (bytesAvailable > SER_PACKET_SIZE)
            {
                // Request SER_PACKET_SIZE bytes, don't release the bus
                wireport->requestFrom((uint8_t)deviceaddress,
                                      (uint8_t)SER_PACKET_SIZE,
                                      (uint8_t)false);
                ...
                // Decrease the number of bytes available by SER_PACKET_SIZE
                bytesAvailable -= SER_PACKET_SIZE;
            }
            // Request remaining bytes, release the bus
            wireport->requestFrom((uint8_t)deviceaddress, (uint8_t)bytesAvailable);
...
void IridiumSBD::check9603pins()
{
    ...
    // Send data and release the bus (the 841 (WireS) doesn't like it if the Master holds the bus!)
    wireport->endTransmission();

    // Request one byte from the IO register
    if (wireport->requestFrom((uint8_t)deviceaddress, (uint8_t)1) == 1)
    {
...
int IridiumSBD::internalPassThruI2Cread(uint8_t *rxBuffer,
                                        size_t &rxBufferSize, size_t &numBytes)
{
    if (this->asleep)
        return ISBD_IS_ASLEEP;
    ...
    if (bytesAvailable > 0)
    {
        ...
        wireport->beginTransmission((uint8_t)deviceaddress); // Talk to the I2C device
        wireport->write(DATA_REG); // Point to the serial buffer
        // Send data and release the bus (the 841 (WireS) doesn't like it
        // if the Master holds the bus!)
        wireport->endTransmission();
...
int IridiumSBD::internalPassThruI2Cwrite(uint8_t *txData, size_t &txDataSize)
{
    ...
    // There are now <= (TINY_I2C_BUFFER_LENGTH - 1) bytes left to send, so send
    // them and then release the bus
    wireport->beginTransmission((uint8_t)deviceaddress);
