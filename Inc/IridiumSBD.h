...
class IridiumSBD
{
    public:
       ...
       // Weak functions to configure and set the sleep pin - user can overwrite
       // with a custom functions if required
       void configureSleepPin() __attribute__((weak));
       void setSleepPin(uint8_t enable) __attribute__((weak));

       // Weak functions to begin and end the Serial port after
       // power(true) and before power(false)
    ...
    private:
        ...
        int startupTimeout;
        // The time in millis when the I2C bus was last checked (limits I2C traffic)
        unsigned long lastCheck = 0;
        //Limit checking of new characters to every 5 ms (roughly 10 chars at 19200 baud)
        const uint8_t I2C_POLLING_WAIT_MS = 5;
        ...
        bool waitForATResponse(char *response=NULL, int responseSize=0,
                               const char *prompt=NULL, const char *terminator="OK\r\n");
        ...
        int  doSBDIX(uint16_t &moCode, uint16_t &moMSN, uint16_t &mtCode, uint16_t &mtMSN,
                     uint16_t &mtLen, uint16_t &mtRemaining);
        ...
        // The serial length regsiter: 2 bytes (MSB, LSB) indicating how many
        // serial characters are available to be read
        #define LEN_REG 0xFD
        // The serial data register: used to read and write serial data from/to the 9603N
        #define DATA_REG 0xFF

        // Create the IO 'register'
        // A '1' in any of the bits indicates that the pin
        // is ON (not necessarily that it is HIGH!)
        byte IO_REGISTER;
        ...
        // 9603N Ring Indicator _flag_ : Read / Write (Set if RI has been seen,
        // cleared by writing a 0 to this bit)
        const uint8_t IO_RI      = (1 << 3);
        const uint8_t IO_NA      = (1 << 4); // 9603N Network Available : Read only
        const uint8_t IO_PGOOD   = (1 << 5); // LTC3225 PGOOD : Read only
        // ATtiny841 low power mode : Read / Write : Set to enable low power mode
        const uint8_t IO_LOW_PWR = (1 << 6);
